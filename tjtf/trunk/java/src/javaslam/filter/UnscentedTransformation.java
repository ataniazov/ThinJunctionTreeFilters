...
public class UnscentedTransformation extends Linearization {
  ...
  public UnscentedTransformation(NoisyVectorFunction f, 
    ...
    Flops.count(Flops.solve(sigmaXX.getRowDimension(),
			    sigmaXX.getColumnDimension(),
			    sigmaXY.getColumnDimension(),
			    true) +
		Flops.add(muY.getRowDimension(), 
			  muY.getColumnDimension()) +
		Flops.mult(B.getRowDimension(), 
			   B.getColumnDimension(),
			   muX.getColumnDimension()) +
		Flops.add(sigmaYY.getRowDimension(), 
  ...
  private Matrix cov(Matrix A, Matrix B) {
    ...
    Flops.count(A.getRowDimension() * A.getColumnDimension() +
		Flops.mult(C.getRowDimension(),
  ...
  private double[][] getSigmaPoints(Gaussian p) {
    ...
    Flops.count(1 + 2 * d * nsp + Flops.chol(sigma.getRowDimension()));
