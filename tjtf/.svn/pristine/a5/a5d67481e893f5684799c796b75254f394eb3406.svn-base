...
public class KalmanFilter implements Filter {
  ...
  public void measurement(ListSet vars, double[] y0, double[][] C, 
    ...
    Flops.count(Flops.mult(CM.getRowDimension(), 
			   ...
			   muV.getColumnDimension()) +
		Flops.add(y0M.getRowDimension(), 
			  y0M.getColumnDimension()) +
		Flops.mult(sigmaVV.getRowDimension(), 
			   sigmaVV.getColumnDimension(), 
			   CM.getRowDimension()) +
		Flops.mult(CM.getRowDimension(), 
			   CM.getColumnDimension(), 
			   sigmaVY.getColumnDimension()) +
    ...
    Flops.count(Flops.mult(sigmaUV.getRowDimension(), 
			   sigmaUV.getColumnDimension(), 
			   CM.getRowDimension()));
    // Compute the Kalman gain matrix K = sigmaUY / sigmaYY
    Matrix K = sigmaYY.solve(sigmaUY.transpose()).transpose();
    Flops.count(Flops.solve(sigmaYY.getRowDimension(), 
			    sigmaYY.getColumnDimension(), 
			    sigmaUY.getRowDimension(), true));
    ...
    Flops.count(Flops.add(yM.getRowDimension(), 
			  yM.getColumnDimension()) + 
		Flops.mult(K.getRowDimension(), 
    ...
    for (int i = 0; i < n; i++)
      for (int j = i + 1; j < n; j++)
	esUU[j][i] = esUU[i][j];
    Flops.count(Flops.mult(K.getRowDimension(), 
			   K.getColumnDimension(), 
			   sigmaUY.getColumnDimension()) +
		Flops.add(n, n));
  }
  ...
  public void joseph(ListSet vars, double[] y0, double[][] C, 
    ...
    Flops.count(n + Flops.mult(K.getRowDimension(), 
			       K.getColumnDimension(), 
			       CM.getColumnDimension()) +
		2 * Flops.mult(n, n, n) +
		Flops.add(n, n) +
  ...
  public void time(ListSet vars, double[] x0, 
    ...
    Flops.count(Flops.mult(AM.getRowDimension(), 
			   AM.getColumnDimension(), 
			   mu.getColumnDimension()) +
		Flops.add(x0M.getRowDimension(), 
			  x0M.getColumnDimension()) +
		Flops.mult(sigma.getRowDimension(), 
			   sigma.getColumnDimension(), 
			   AM.getRowDimension()) +
		Flops.mult(AM.getRowDimension(), 
			   AM.getColumnDimension(), 
			   tmp.getColumnDimension()) +
		Flops.add(QM.getRowDimension(), 
			  QM.getColumnDimension()) +
		Flops.mult(AM.getRowDimension(),
			   AM.getColumnDimension(),
			   CC.getColumnDimension()));
  }
