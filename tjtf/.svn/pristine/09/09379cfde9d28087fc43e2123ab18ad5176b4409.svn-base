...
public class Gaussian {
  ...
  public Gaussian reparameterize(boolean inPlace) {
    ...
    if (TJTF.verbose) 
      TJTF.say("Inverting " + dim + "-by-" + dim + " matrix ");
    ...
    // Count flops.
    Flops.count(Flops.inv(dim, true) + 
		Flops.mult(dim, dim, 1));
    return q;
  }
  ...
  public Gaussian marginalize(Set vars, boolean inPlace) {
	...
	for (int i = 0; i < lambda_mm.getRowDimension(); i++)
      ...
      */
      ...
      if (TJTF.verbose) 
	TJTF.say("Inverting " + lambda_mm.getRowDimension() + "-by-" + 
		 lambda_mm.getColumnDimension() + 
		 " matrix to compute a marginal over " + vars);
      if (false) {
	...
	lambda_div = lambda_mm.solve(lambda_mr).transpose();
      ...
      // Count the flops.
      Flops.count(Flops.solve(lambda_mm.getRowDimension(), 
			      lambda_mm.getColumnDimension(), 
			      lambda_mr.getColumnDimension(), 
			      true) +
		  Flops.mult(lambda_div.getRowDimension(), 
			     lambda_div.getColumnDimension(),
			     eta_m.getColumnDimension()) +
		  Flops.add(m_eta.getRowDimension(), 
			    m_eta.getColumnDimension()) +
		  Flops.mult(lambda_div.getRowDimension(), 
			     lambda_div.getColumnDimension(),
			     lambda_mr.getColumnDimension()) +
		  Flops.add(m_lambda.getRowDimension(), 
			    m_lambda.getColumnDimension()));
  ...
  public Gaussian condition(ListSet cvars, Matrix obs, boolean inPlace) {
    ...
    if (isCanonical()) {
      ...
      // Count the flops.
      Flops.count(Flops.mult(lambda_rc.getRowDimension(),
			     lambda_rc.getColumnDimension(), 
			     obs.getColumnDimension()));
	...
	return this;
      ...
      // error.  Thus, we use the standard solver.
      if (TJTF.verbose) 
	TJTF.say("Inverting " + sigma_cc.getRowDimension() + "-by-" + 
		 sigma_cc.getColumnDimension() + 
		 " matrix to compute a conditional over " + cvars);
      if (false) {
	...
	sigma_div = sigma_cc.solve(sigma_cr).transpose();
      ...
      // Count the flops.
      Flops.count(Flops.solve(sigma_cc.getRowDimension(), 
			      sigma_cc.getColumnDimension(), 
			      sigma_cr.getColumnDimension(), 
			      true) +
		  Flops.add(mu_c.getRowDimension(), 
			    mu_c.getColumnDimension()) +
		  Flops.mult(sigma_div.getRowDimension(), 
			     sigma_div.getColumnDimension(),
			     mu_c.getColumnDimension()) +
		  Flops.add(c_mu.getRowDimension(), 
			    c_mu.getColumnDimension()) +
		  Flops.mult(sigma_div.getRowDimension(), 
			     sigma_div.getColumnDimension(),
			     sigma_cr.getColumnDimension()) +
		  Flops.add(c_sigma.getRowDimension(), 
			    c_sigma.getColumnDimension()));
      // Return the conditional Gaussian.
      if (inPlace) {
  ...
  public Gaussian times(Gaussian p, boolean inPlace) {
    ...
    if (isMoment()) {
      ...
      Flops.count(Flops.add(k, 1) + Flops.add(k, k));
    }
    return q;
  }
  ...
  public double entropy() {
    // Count the flops
    Flops.count(Flops.det(mP.getRowDimension(), true) + 5 + Flops.log());
    if (isMoment())
  ...
  public double kl(Gaussian q) {
    ...
    // the inverses above.
    Flops.count(Flops.add(n, 1) +        // muP - muQ
		Flops.mult(n, 1, n) +    // (muP - muQ)(muP - muQ)'
		Flops.add(n, n) +        // sigmaP + (muP - muQ)(muP - muQ)'
		Flops.mult(n, n, n) +    // lambdaQ * (...)
		Flops.trace(n) +         // trace(...)
		2 * Flops.det(n, true) + // det(sigmaP), det(sigmaQ)
		2 * Flops.log() +        // log(det(sigmaP)), log(det(sigmaQ))
		4);                      // scalar flops
    return kl;
  }
  ...
  public Matrix sample(int n, Random r) {
    ...
    for (int i = 0; i < getDimension(); i++)
      for (int j = 0; j < n; j++)
	elts[i][j] += mu.get(i, 0);
    Flops.count(Flops.randnorm(n, getDimension()));
    return samples;
  }
  ...
  public String toString() {
    ...
    b.append("\nvector parameter:\n");
    b.append(TJTF.toString(vP));
    b.append("matrix parameter:\n");
    b.append(TJTF.toString(mP));
    return b.toString();
  }
